...
HOOKS_JSON_SRC="$SCRIPT_DIR/hooks/ai-harness-hooks.json"
AUDIO_SRC="$SCRIPT_DIR/../audio"
COPILOT_HOME="${COPILOT_HOME:-$HOME/.copilot}"
SKILLS_DEST="$COPILOT_HOME/skills"
AGENTS_DEST="$COPILOT_HOME/agents"
HOOKS_DEST="$COPILOT_HOME/hooks"
AUDIO_DEST="$COPILOT_HOME/audio"
NODE_PATH_DEST="$COPILOT_HOME/ai-harness-node-path.txt"
...
# Hooks: *.mjs / *.ps1 from claude/hooks/ (shared play-sound script)
hook_files=()
if [ -d "$HOOKS_SRC" ]; then
...
if [ ${#skills[@]} -eq 0 ] && [ ${#agents[@]} -eq 0 ] \
   ...
   && [ ! -f "$HOOKS_JSON_SRC" ]; then
  echo "nothing to install - no skills, agents, hooks, audio, or hooks config found" >&2
  exit 1
fi
...
write_node_path_file() {
  ...
  if [ -z "$node_bin" ]; then
    echo "warning: node not found during sync; Copilot hooks will still depend on runtime PATH"
    return 0
  fi
...
install_hooks_json() {
  ...
  if [ -L "$dest" ] || [ -e "$dest" ]; then
    if [ -L "$dest" ] && [ "$src" -ef "$dest" ]; then
      ...
      echo "skip (name exists, leaving alone): $dest"
    fi
    return 0
  fi
...
if [ "$UNINSTALL" -eq 1 ]; then
  ...
  uninstall_one "$HOOKS_DEST/ai-harness-hooks.json"
...
hooks_json_installed=0
if [ -f "$HOOKS_JSON_SRC" ]; then
  echo "hooks config: $HOOKS_JSON_SRC -> $HOOKS_DEST/ai-harness-hooks.json"
  install_hooks_json "$HOOKS_JSON_SRC" "$HOOKS_DEST/ai-harness-hooks.json"
  hooks_json_installed=1
  echo
fi

total=$(( ${#skills[@]} + ${#agents[@]} + ${#hook_files[@]} + ${#audio_files[@]} + hooks_json_installed ))
...
if [ "$hooks_json_installed" -eq 1 ]; then
  echo "  hooks.json: $HOOKS_DEST/ai-harness-hooks.json"
fi
